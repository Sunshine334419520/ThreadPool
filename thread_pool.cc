...
thread_local internal::WorkStealinggQueue*
...
void ThreadPool::Start() {
	...
	try {
		for (unsigned int i = 0; i < thread_num_; i++) {
			queues_.push_back(std::unique_ptr<internal::WorkStealinggQueue>(
				new internal::WorkStealinggQueue));
			threads_.push_back(
...
void ThreadPool::JoinAll() {
	...
	queues_.swap(std::vector<std::unique_ptr<internal::WorkStealinggQueue>>());
